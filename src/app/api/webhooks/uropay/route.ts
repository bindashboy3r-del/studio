 ...
 * This credits the user's balance automatically when a payment is successful.
 */
export async function POST(req: Request) {
  try {
    const data = await req.json();
    
    // Standard UroPay callback params
    ...
    if (status === 'success' || status === 'TXN_SUCCESS') {
      ...
      if (!snapshot.empty) {
        ...
        const totalToCredit = parseFloat(amount) * (1 + bonusPct / 100);
        ...
        batch.set(notifRef, {
          ...
          message: `₹${totalToCredit.toFixed(0)} added via UPI. (Includes ${bonusPct}% bonus)`,
          read: false,
          createdAt: serverTimestamp()
        });

        await batch.commit();
        console.log(`Successfully credited ${totalToCredit} to user ${userId}`);
      }
    }
