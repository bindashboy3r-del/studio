    ...
    try {
      if (withdrawType === 'Wallet') {
        ...
        batch.set(doc(collection(db, "withdrawals")), {
          userId: user.uid, amount: amt, type: 'Wallet', status: 'Approved', createdAt: serverTimestamp()
        });
        await batch.commit();
      ...
      <main className="max-w-md mx-auto p-4 space-y-6 mt-2">
        <div className="bg-[#312ECB] rounded-[2.5rem] p-8 text-white relative overflow-hidden shadow-2xl">
          <div className="relative z-10 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-[10px] font-black uppercase text-white/60 tracking-widest">Commission Dashboard</p>
                <h2 className="text-2xl font-black uppercase">5% Daily Rewards</h2>
              </div>
            </div>
            <div className="pt-4 border-t border-white/10 flex justify-between items-end">
              <div>
                <p className="text-[9px] font-black text-white/50 uppercase mb-1">Referral Balance</p>
        ...
        <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-gray-100 space-y-6">
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              <div className="flex-1 bg-slate-50 p-4 rounded-2xl border border-slate-100 font-bold text-xs text-slate-500 truncate">{referralLink}</div>
              <Button onClick={handleCopyLink} size="icon" className="h-12 w-12 rounded-2xl bg-[#312ECB] shadow-md"><Copy size={18} /></Button>
            </div>
            <Button onClick={() => { window.open(`https://wa.me/?text=${encodeURIComponent(`🚀 Hey! Build your social media with SocialBoost. Use my link to join: ${referralLink}`)}`, '_blank'); }} className="w-full h-12 bg-[#25D366] rounded-2xl font-black text-[10px] uppercase gap-2 shadow-lg"><Share2 size={16} /> Share on WhatsApp</Button>
          </div>
        </div>
        ...
        <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-gray-100 space-y-6">
          ...
          <div className="space-y-4">
            <Input type="number" placeholder="Enter Amount (Min ₹10)" value={withdrawAmount} onChange={(e) => setWithdrawAmount(e.target.value)} className="h-12 bg-slate-50 border-none rounded-2xl px-5 font-black text-sm text-[#111B21]" />
            {withdrawType === 'UPI' && <Input placeholder="Enter your UPI ID (e.g. user@paytm)" value={upiId} onChange={(e) => setUpiId(e.target.value)} className="h-12 bg-slate-50 border-none rounded-2xl px-5 font-black text-sm text-[#111B21]" />}
            <Button onClick={handleWithdraw} disabled={isWithdrawing || !withdrawAmount} className="w-full h-12 bg-[#312ECB] text-white rounded-2xl font-black text-[10px] uppercase shadow-lg gap-2">{isWithdrawing ? <Loader2 size={16} className="animate-spin" /> : <ArrowRightLeft size={16} />} {withdrawType === 'Wallet' ? "Transfer to Wallet" : "Request Payout"}</Button>
          </div>
        </div>
          ...
          <div className="space-y-3">
            {transactions && transactions.length > 0 ? transactions.map((t: any) => (
              <div key={t.id} className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex items-center justify-between">
                <div><p className="text-[11px] font-black text-slate-900 uppercase">Deposit from {t.fromUserName || 'Referred User'}</p><p className="text-[8px] font-bold text-slate-400 uppercase">₹{t.depositAmount} added</p></div>
                <Badge className="bg-emerald-50 text-emerald-600 border-none text-[10px] font-black">+₹{t.commissionAmount.toFixed(2)}</Badge>
              </div>
