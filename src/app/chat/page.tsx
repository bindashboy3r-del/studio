...
export default function ChatPage() {
  ...
  const handleBulkLinksFromCard = async (links: string[]) => {
    ...
    botReply(`📊 Quantity PER LINK for ${selectedService.name}? (Min ${selectedService.minQuantity})`, ["500", "1000", "5000", "10000"]);
  };
      ...
      case 'choosing_service':
        ...
        if (selected) {
          if (currentOrder.type === 'bulk') {
            ...
            setCurrentOrder(prev => ({ ...prev, items: [{ service: selected, quantity: 0, link: '' }] }));
            setChatState('entering_quantity');
            botReply(`📊 Quantity for ${selected.name}? (Min ${selected.minQuantity})`, ["500", "1000", "5000", "10000"]);
          }
        }
        break;
