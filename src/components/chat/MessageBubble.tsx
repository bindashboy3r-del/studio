...
export function MessageBubble({ 
  ...
  return (
    <div className={cn("flex w-full mb-2", isUser ? "justify-end" : "justify-start")}>
      <div className={cn("max-w-[94%] px-3 py-2 relative shadow-3d-sm", isUser ? "bubble-user rounded-[16px] rounded-tr-none" : "bubble-bot rounded-[16px] rounded-tl-none")}>
        {isSuccessCard && successDetails ? (
          <div className="space-y-2 min-w-[180px]">
            ...
            <div className="bg-slate-50 dark:bg-slate-900/50 p-2 rounded-lg text-[9px] font-bold space-y-1 text-slate-800 dark:text-slate-100 border dark:border-slate-700 shadow-3d-pressed">
            ...
            <Button onClick={() => onOptionClick?.("Main Menu")} className="w-full h-8 bg-[#312ECB] text-[8px] uppercase font-black rounded-lg shadow-3d-sm active:shadow-3d-pressed">Back to Menu</Button>
          </div>
        ) : isPaymentCard ? (
          <div className="space-y-2.5 min-w-[200px]">
            ...
            <div className="bg-slate-50 dark:bg-slate-900/50 p-2 rounded-lg flex flex-col items-center gap-2 border border-slate-100 dark:border-slate-800 shadow-3d-pressed">
              ...
              <div className="flex w-full gap-1.5">
                <Button 
                  ...
                  variant="outline" 
                  className="flex-1 h-7 text-[7px] uppercase font-black rounded-md px-0 shadow-3d-sm active:shadow-3d-pressed"
                >
                ...
                <Button 
                  ...
                  variant="outline" 
                  className="flex-1 h-7 text-[7px] uppercase font-black text-[#312ECB] rounded-md px-0 shadow-3d-sm active:shadow-3d-pressed"
                >
            ...
            <div className="space-y-2">
              <div className="space-y-1.5">
                <div className="space-y-0.5">
                  ...
                  <Input 
                    ...
                    className="h-8 rounded-md bg-slate-50 dark:bg-slate-800 text-[10px] border-none shadow-3d-pressed font-bold" 
                  />
                </div>
                ...
                <div className="space-y-0.5">
                  ...
                  <Input 
                    ...
                    className="h-8 rounded-md bg-slate-50 dark:bg-slate-800 text-[10px] border-none shadow-3d-pressed font-black tracking-widest" 
                  />
                </div>
              </div>
              ...
              <Button 
                ...
                className="w-full h-9 bg-[#312ECB] font-black text-[8px] uppercase tracking-widest rounded-md shadow-3d-sm active:shadow-3d-pressed"
              >
                Submit Payment Info
              </Button>
            </div>
          ...
          <div className="space-y-2 min-w-[210px]">
            ...
            <div className="max-h-[120px] overflow-y-auto space-y-1 pr-1 custom-scrollbar">
              {comboItems.map((item, idx) => {
                const s = dynamicServices?.find(sv => sv.id === item.serviceId);
                return (
                  <div key={idx} className="bg-slate-50 dark:bg-slate-800 p-1.5 rounded-lg flex flex-col gap-1 border dark:border-slate-700 shadow-3d-pressed">
                    ...
                    <div className="flex items-center gap-2">
                      <Input type="number" value={item.quantity} onChange={(e) => setComboItems(prev => prev.map((it, i) => i === idx ? { ...it, quantity: parseInt(e.target.value) || 0 } : it))} className="h-6 w-full text-[8px] bg-white dark:bg-slate-900 border-none px-1.5 shadow-inner" />
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild><Button variant="outline" className="w-full h-7 text-[7px] font-black uppercase border-slate-200 text-[#312ECB] rounded-lg shadow-3d-sm active:shadow-3d-pressed"><Plus size={8} /> Add Service</Button></DropdownMenuTrigger>
              <DropdownMenuContent className="w-[140px] max-h-[120px] overflow-y-auto rounded-lg p-1 shadow-3d">
            ...
            <div className="space-y-0.5">
              ...
              <Input placeholder="Profile/Post Link" value={comboLink} onChange={(e) => setComboLink(e.target.value)} className="h-8 bg-slate-50 dark:bg-slate-800 text-[10px] rounded-md border-none shadow-3d-pressed" />
            </div>
            <div className="bg-emerald-50 dark:bg-emerald-950 p-1.5 rounded-lg text-center shadow-3d-pressed">
            ...
            <Button onClick={() => onComboSubmit?.(comboItems, comboLink)} disabled={!comboLink || comboItems.length === 0} className="w-full h-9 bg-[#312ECB] font-black text-[8px] uppercase rounded-md shadow-3d-sm active:shadow-3d-pressed">Confirm Bundle</Button>
          </div>
        ) : isBulkLinkCard ? (
          <div className="space-y-2 min-w-[200px]">
            ...
            <div className="space-y-1">
              <div className="flex gap-1.5">
                <Input placeholder="Add Link..." value={currentBulkLink} onChange={(e) => setCurrentBulkLink(e.target.value)} className="h-8 bg-slate-50 dark:bg-slate-800 rounded-md border-none flex-1 text-[10px] shadow-3d-pressed" />
                <Button onClick={() => { if(currentBulkLink) { setBulkLinks([...bulkLinks, currentBulkLink]); setCurrentBulkLink(""); } }} className="h-8 w-8 bg-[#312ECB] rounded-md shadow-3d-sm active:shadow-3d-pressed"><Plus size={14}/></Button>
              </div>
              <div className="max-h-[80px] overflow-y-auto space-y-1 pr-1 custom-scrollbar">
                {bulkLinks.map((l, i) => (
                  <div key={i} className="flex items-center justify-between bg-slate-50 dark:bg-slate-800 p-1 rounded-lg text-[7px] font-bold shadow-3d-pressed">
            ...
            <Button onClick={() => onBulkLinksSubmit?.(bulkLinks)} disabled={bulkLinks.length === 0} className="w-full h-9 bg-[#312ECB] text-[8px] font-black uppercase rounded-md shadow-3d-sm active:shadow-3d-pressed">Continue ({bulkLinks.length})</Button>
          </div>
        ) : isWalletCard ? (
          <div className="space-y-2.5 min-w-[180px]">
             <div className="flex flex-col items-center gap-1.5 p-3 bg-blue-50 dark:bg-blue-900/20 rounded-xl border border-blue-100 dark:border-blue-900/30 shadow-3d-pressed">
                <div className="w-8 h-8 bg-white dark:bg-slate-800 rounded-lg flex items-center justify-center shadow-3d-sm">
             ...
             <Button onClick={() => onWalletSubmit?.(link)} className="w-full h-9 bg-[#312ECB] font-black text-[8px] uppercase rounded-md shadow-3d-sm active:shadow-3d-pressed">Confirm Payment</Button>
          </div>
        ) : (
        ...
        {options && !isPaymentCard && !isSuccessCard && !isComboCard && !isBulkLinkCard && !isWalletCard && (
          <div className="mt-3 space-y-1">
            {options.map((opt, i) => (
              <button key={i} onClick={() => onOptionClick?.(opt)} className="w-full bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 p-2.5 rounded-lg flex items-center justify-between group shadow-3d-sm active:shadow-3d-pressed transition-all">
